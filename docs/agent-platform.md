...
- Built + **live-validated**: SDK in the bundle, browserless runtime, MCP server, the
  kibitzer on the SDK.
- Built (in code, pending a 2-device live test before deploy): the **capability / consent /
  audit layer** — per-participant grants, engine-enforced perceive/act + per-peer media
  gating, host consent panel + revoke, and egress disclosure (see §2 and
...
- Built (in code, pending a 2-device live test): **self-held-key agent admission**
