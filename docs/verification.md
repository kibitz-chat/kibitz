...
`room.ts:196-219`, `Widget.tsx` (`withAgentGate`).
...
- **`mail`** — a mailed code to an email (via the email-code backend, §4.5) → that exact
  ...
  (`joinGateRuntime.ts:231`). *(The email backend has shipped — see §4.5 — so this is no longer
...
credential the link committed*, **NOT** to being the coordinator (`room.ts:392`,
`useLobby.ts:105-110`). This is what fixes the "coup" (a stranger who became coordinator used to
...
`room.ts:560-561` send → `room.ts:528 handleMod` → `room.ts:492 enactMod`;
`hostKey.ts:112 verifyHostCommand`).
...
re-claims) — `room.ts:364`, `room.ts:645-648`.

The room link commits **one of four** host tiers, chosen at creation (the CreatePage "Room
admin (host)" 4-way chooser — `CreatePage.tsx:58 hostTier`):

| Tier | Link params | How admin is claimed | Trust property |
|------|-------------|----------------------|----------------|
| **None** (open) | — | — | **No admin at all.** With no host key committed, every `mod` command is dropped (`room.ts:527-529` "open room = no admin"). A room with no host has no live moderation — a behavior change from "the first peer moderates." |
| **Soft name** | `ghn` (+ `gl`) | Whoever announces under the committed display name becomes host (`room.ts:370 matchHostByName`, `useCall.ts:1184 claimHostByName`). First-match-holds; reclaim-by-name after the host disconnects. | **Spoofable** — no crypto; any link-holder can use the name. For "I'm first in, hold the door, let the agent in, then admit everyone" convenience, not defense. `gl=1` starts the waiting room ON (`room.ts:297 lastLobbyOn`), which holds a stranger *before* they can announce a name. |
| **OIDC email** | `gho` + `gc` | Host signs in (OIDC); every peer verifies every peer's cert-bound ID token peer-to-peer, the authority matches the committed email **locally**, and `declareHost`s it (`room.ts:745-751 declareHostFn`, `room.ts:99-100 declareHost`, `useCall.ts:1194`). Signing in *is* claiming. | **Un-spoofable + portable** — sign in on any device, nothing stored. Needs the internet (JWKS) and a Google OAuth client id in the link (public, like verified rooms). The room itself stays open; only *admin* is gated to the email. |
| **Password key** | `gh` + `ghk` | Enter the host password → unseal the private key from `ghk` → sign cert-bound commands the coordinator verifies against `gh` (`hostKey.ts:52-138`, `useCall.ts:1125 claimHost` → `useCall.ts:1147 hostModerate`). | **Un-spoofable, no provider, moderates from any seat, migration-safe** (each command is individually signed). The sealed key rides the **public** link, so a weak password is **offline-brute-forceable** (PBKDF2 only slows it; no server to throttle) — use a passphrase (`hostKey.ts:10-11`; the create UI nudges a strong one). |

**Commands** (`hostKey.ts:31-32 HostOp`/`HOST_OPS`):
...
on another connection, room, or later (`hostKey.ts:36-48 HostCommandPayload`,
`hostKey.ts:112-138 verifyHostCommand`). The `remoteFp` cert binding is *optional* so a
coordinator that is itself the host can self-inject a command without a remote fingerprint
(`room.ts:528-545`, the `sendModFn` self-inject path).
...
room **loses its admin until someone re-claims it** (`room.ts:645-648`) — admin does **not**
silently transfer to the next coordinator. **No host committed = no admin**, ever
(`room.ts:527-529`).

**Precedence.** A committed host **key** wins over a committed **name**/**email** — the name and
email tiers are disabled when a host key is present (`room.ts:265-271`). Host fields are read for
