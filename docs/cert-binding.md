...
Think of a "Sign in with Google" token as a **sealed letter from Google that says "the person holding
this is alice@acme.com."** Google's signature on it is unforgeable — but the letter is still just
*paper*: anyone who gets a copy can wave it around and claim to be Alice. And because a Kibitz call has
**no server** standing at the door to check faces, a copied letter looks exactly like the real one.

Cert-binding fixes that by **gluing the letter to the exact line you're talking on.** Every direct
connection between two browsers has its own unique, un-fakeable fingerprint — you cannot lie about which
connection you're actually on. When Alice signs in, her letter is **stamped with the fingerprint of
*her* connection.** So when you receive a letter claiming to be Alice, you don't just read it — you
check that the fingerprint stamped on it matches the fingerprint of the call you're **actually** having
with her. A stolen copy presented over a *different* connection carries the *wrong* stamp, and the check
fails on the spot.

The upshot: the credential stops being a transferable *pass* and becomes a **seal on this one
conversation** — it proves identity only on the exact connection it was minted for, and each browser
verifies it entirely by itself, with no server anywhere in the loop. The rest of this doc is how that's
actually done.
...
**In plain terms — a man-in-the-middle is trapped with no exit.** To get between Alice and Bob,
"Mallory" must make Bob believe *he* is Alice — i.e. present **Alice's identity on Mallory's own
connection**. He has exactly **two moves, and both fail**: (1) **replay Alice's real token** — but it
names *Alice's* fingerprint, while the connection Bob is actually on carries *Mallory's*, so the check
rejects it; or (2) **forge a token naming his own fingerprint under Alice's name** — but that needs the
issuer's signature (Google's / ours), which he can't fake. There is **no third move.** And he can't cheat
by tampering with the signaling, because each peer reads the fingerprint from the connection it
**actually handshook**, not from anything Mallory relays. So it isn't that a MITM is *hard* — the one
thing he'd need (Alice's fingerprint on his own line) is **cryptographically unavailable to him.** The
rest of this section is that same point, precisely:
...
`signHostCommand`), and every verifier rejects it unless that fingerprint matches
the connection it actually handshook with (the `canonicalFingerprint(payload.fp)` check in
`verifyHostCommand`). So a captured `mod` command is just
...
| **Host moderation commands** | `hostKey.ts` `signHostCommand` → `verifyHostCommand` | a `mod` command (lock/kick/admit…) is enacted only if its signed payload's `fp` matches the live `remoteFp` via `canonicalFingerprint` (`signHostCommand`/`verifyHostCommand`), plus room-bound + fresh — so a captured command can't be replayed on another connection (see §4) |
| **OIDC host election** | `useCall.getIdentity` → `room.ts` `declareHost` | a member is marked the verified host only after the authority verifies their **cert-bound** OIDC token against the committed host email (`nonce = hash(fingerprint)`, the same binding applied to host election, `room.ts` `declareHostFn`) |
