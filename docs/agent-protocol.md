...
2. **The engine** enforces the grant *per peer*, so a tampered agent client still can't
   ...
   - **act = receiver-side drop** — every honest peer ignores chat/app/pay/ink from a peer
     whose grant lacks `send-chat` (the `send-chat` check in the dispatch handler before
     delivery, [`src/react/useCall.ts:704`](../src/react/useCall.ts)), and logs it to the host
     audit ([`useCall.ts:705`](../src/react/useCall.ts)).
