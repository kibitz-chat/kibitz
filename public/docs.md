...
- **Post an image / file into the chat** (so a headless agent whose tool produced one can put it in the room): `sendImage(payload, to?)` / `sendFile(payload, to?)`, where `payload` is `{ mime, data, name? }` and `data` is **base64** (a File object isn't handy out-of-page). Both ride the chunked content transfer — no inline-image size cap; an image renders inline, anything else as a download. With `to` it's private to that one peer; broadcast is withheld from peers not granted `read-media` (image) / `read-files` (file).
...
Every participant carries a **`Grant`** of what it may **perceive** (content that flows to it) and **act** (what it emits): `{ perceive: ('see-screen'|'hear-audio'|'read-chat'|'read-roster'|'receive-directed'|'read-media'|'read-files')[], act: ('send-chat'|'speak'|'act')[], backend?, egress?, expiresAt? }`. **Humans default to full; agents (`meta.role==='agent'`) default to read-only** — they perceive chat/roster/directed data, but receive **no media** and can **act nothing**.
