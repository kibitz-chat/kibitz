...
export interface CreditVerifyOpts {
  ...
  kind?: string
  /** If set, the credential's `room` claim MUST equal this. SCOPES the credit to ONE room so a captured
   *  credential can't be replayed into a different credit-gated room trusting the same issuer. Fail-closed:
   *  a credential with no `room` claim is rejected when a room is required (the kibitz issuer always mints
   *  it — renew requires `room`). Omit to keep the old room-agnostic behavior. */
  room?: string
}
...
export async function verifyCreditCredential(jwt: string, opts: CreditVerifyOpts): Promise<CreditResult> {
  ...
  // 4b. Room scope (when required). Without this, a credit minted for room X replays into any credit-gated
  //     room trusting the same issuer. Fail-closed: a missing `room` claim can't be scoped, so it's rejected.
  if (opts.room != null && claims.room !== opts.room) return { ok: false, reason: 'wrong room' }
