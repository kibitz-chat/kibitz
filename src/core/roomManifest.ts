...
/** Is this an AGENT-ONLY room — agent(s) pre-authorized by key, but NO human allow-list (no members,
 *  no domains)? Then there is NO human gate: humans join OPEN (the gate verifies only the agent, by its
 *  cert-bound key). The authority uses this to admit a credential-less human directly instead of holding
 *  them (room.ts `openHumans`); the Widget passes it from the verified manifest. Pure — unit-testable, and
 *  the single source of truth for "humans are open here" so room.ts + the Widget can't drift. */
export function humansOpenForManifest(m: Pick<RoomManifest, 'members' | 'domains' | 'agentKeys'>): boolean {
  return (m.agentKeys?.length ?? 0) > 0 && !m.members?.length && !m.domains?.length
}
