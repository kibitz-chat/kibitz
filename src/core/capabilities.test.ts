...
describe('defaultGrant — humans full, agents least-privilege', () => {
  ...
  it('vision (read-media) + files (read-files) are human defaults but agent opt-ins', () => {
    // A human sees shared images + receives files out of the box; an agent reads the text chat but
    // NOT image bytes or files until the host explicitly grants read-media / read-files.
    expect(canPerceive(defaultGrant('human'), 'read-media')).toBe(true)
    expect(canPerceive(defaultGrant('human'), 'read-files')).toBe(true)
    expect(canPerceive(defaultGrant('agent'), 'read-media')).toBe(false)
    expect(canPerceive(defaultGrant('agent'), 'read-files')).toBe(false)
    // distinct from read-chat: an agent admitted to the chat still can't see images or get files.
    expect(canPerceive(defaultGrant('agent'), 'read-chat')).toBe(true)
  })
})
