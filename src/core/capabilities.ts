...
// PERCEIVE: `read-chat`/`read-roster`/`receive-directed`/`read-media` gate DATA (mesh content, useCall);
// the two MEDIA perceives `see-screen`/`hear-audio` gate TRACKS — withheld per-peer in the mesh by
// swapping a flowing placeholder onto that peer's connection (mesh.ts `gatedTrack`/`setMediaGate`).
// `read-media` gates inline IMAGE content (a shared photo / picked image, ContentMsg `k:'img'`) — a
// distinct cap from `read-chat` so an agent can be admitted to the text conversation WITHOUT also being
// fed image bytes (vision is opt-in, granted explicitly). Camera video has no perceive — it's the
// always-on presence lane — so it isn't capability-gated.
// `read-media` gates inline IMAGES, `read-files` gates FILE transfers (ContentMsg `xbegin`+ of kind
// 'image'/'file') — both distinct from `read-chat` so an agent can read the text conversation without
// being fed image bytes or arbitrary files (each is opt-in, granted explicitly).
export type Perceive = 'see-screen' | 'hear-audio' | 'read-chat' | 'read-roster' | 'receive-directed' | 'read-media' | 'read-files'
export type Act = 'send-chat' | 'speak' | 'act'
export type Capability = Perceive | Act
export type ParticipantKind = 'human' | 'agent'

export const ALL_PERCEIVE: readonly Perceive[] = ['see-screen', 'hear-audio', 'read-chat', 'read-roster', 'receive-directed', 'read-media', 'read-files']
 ...
 *  Whist kibitzer's hand), but ACT nothing and get no media. Acting (send-chat/speak/act), media
 *  (see-screen/hear-audio), shared IMAGES (read-media), and FILES (read-files) are granted only by
 *  explicit host consent — so an agent reads the text chat but doesn't receive image bytes or files
 *  until those are turned on.
 *  Perception is disclosed, not withheld, by default (an agent is invited to watch); the dangerous
 *  part — acting — is opt-in. */
