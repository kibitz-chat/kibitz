...
export async function pcFingerprints(pc: PcLike): Promise<{ local: string; remote: string } | null> {
  try {
    // With BUNDLE (the default) every m-line shares ONE DTLS transport, so the first transport we find carries
    // the same cert the media lane uses. A future non-bundled path could split transports — this assumption would
    // then need to pin the media transport explicitly, not just take the first.
    ...
    // WebRTC peers use a SINGLE self-signed cert, so the list length is 1. Any other length means the leaf we'd
    // bind identity to is ambiguous / peer-influenced (the peer controls its own list + order) — refuse rather than
    // hash a possibly-non-leaf cert. This fingerprint underpins cert-binding (oidcBinding) + the SAS, so a
    // wrong-but-plausible value is the dangerous, silent kind. Fail closed: the caller renders "no code" on null.
    if (!remoteCerts || remoteCerts.length !== 1) return null
