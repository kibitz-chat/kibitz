...
export function joinRoom(
  ...
  const becomeAuthority = (transport: AuthorityTransport) => {
    ...
    const gateIdentity = async (id: string, msg: ClientWire) => {
      ...
      if (!jwt && !asn && !credit) return // no credential yet — re-announce after sign-in / with a credential
    ...
    const handleMod = async (token: string, connId: string | null) => {
      ...
      if (connId && remoteFp === null) return // cert not readable yet — the host re-issues
    ...
    const handle = (id: string, msg: ClientWire) => {
      ...
      if (lobbyOn && !admitted.has(id) && msg.t === 'voice' && msg.on) {
