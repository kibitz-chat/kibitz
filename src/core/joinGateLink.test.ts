...
describe('joinGateLink — stateless gate descriptor in the URL', () => {
  ...
  it('round-trips an open-room declare (claim) list — and it survives the open-mode path', () => {
    // The claim list applies to an OPEN room, so it must be read BEFORE decodeGateParams' open-mode early return.
    expect(roundTrip({ mode: 'open', declare: ['alice@acme.com', 'bob@acme.com', 'Guest'] })).toEqual({
      mode: 'open',
      declare: ['alice@acme.com', 'bob@acme.com', 'Guest'],
    })
    expect(decodeGateParams(new URLSearchParams('gd=alice@acme.com,Guest')).declare).toEqual(['alice@acme.com', 'Guest'])
  })
