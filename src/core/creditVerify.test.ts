...
describe('verifyCreditCredential', () => {
  ...
  // H2a — room scoping: a captured credential can't be replayed into a different credit-gated room.
  it('room-scope: accepts when opts.room matches the credential room claim', async () => {
    const r = await verify(await mint(goodClaims()), { room: 'lunar-comet' })
    expect(r.ok).toBe(true)
  })
  it('room-scope: REJECTS a credential minted for a different room', async () => {
    const r = await verify(await mint(goodClaims()), { room: 'other-room' })
    expect(r.ok).toBe(false)
    if (!r.ok) expect(r.reason).toMatch(/room/)
  })
  it('room-scope: fail-closed — a credential with NO room claim is rejected when a room is required', async () => {
    const c = goodClaims()
    delete c.room
    const r = await verify(await mint(c), { room: 'lunar-comet' })
    expect(r.ok).toBe(false)
  })
  it('room-scope: omitting opts.room keeps the old room-agnostic behavior (back-compat)', async () => {
    const r = await verify(await mint(goodClaims())) // no room option
    expect(r.ok).toBe(true)
  })
})
