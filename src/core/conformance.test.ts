...
describe('conformance: wire ContentMsg (additive-only, ignore-unknown)', () => {
  ...
  it('accepts every shipped ContentMsg kind', () => {
    ...
    // Inline image attachments (additive — old peers ignore an unknown 'img' kind).
    expect(asContent({ k: 'img', mime: 'image/jpeg', data: 'AAAA', w: 800, h: 600, dm: true })?.k).toBe('img')
    // Unified chunked content transfer (additive — old peers ignore these + get the legacy chat/img above).
    expect(asContent({ k: 'xbegin', id: 't1', kind: 'file', size: 1000, n: 1, mime: 'application/pdf', name: 'a.pdf' })?.k).toBe('xbegin')
    expect(asContent({ k: 'xchunk', id: 't1', i: 0, data: 'AAAA' })?.k).toBe('xchunk')
    expect(asContent({ k: 'xend', id: 't1' })?.k).toBe('xend')
    expect(asContent({ k: 'xcancel', id: 't1' })?.k).toBe('xcancel')
  })
