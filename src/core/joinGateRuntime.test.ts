...
describe('verified roster — buildVerifiedRoom + manifest-enforced verification', () => {
  ...
  it('agent-only invite manifest (agentKeys, NO human members) keeps humans OPEN — a tokenless human is admitted', async () => {
    // The agent-room bug: the room is invite-gated with an agent key but no human members, and the
    // shared link carries no per-guest token, so every human was denied → split roster. An agent-only
    // manifest means no human gate (the agent is admitted by its own key elsewhere).
    const kp = await generateInviteKeypair()
    const pubKey = await exportInvitePublicKey(kp.publicKey)
    ...
    const verify = await gateVerifierFor({ mode: 'invite', pubKey, manifest }, 'standup', () => NOW)
    expect((await verify(undefined, null)).ok).toBe(true) // a human with NO invite token gets in
  })

  it('but an invite manifest WITH human members still gates a tokenless human (no security regression)', async () => {
    const kp = await generateInviteKeypair()
    const pubKey = await exportInvitePublicKey(kp.publicKey)
    const agentJwk = await exportInvitePublicKey((await generateInviteKeypair()).publicKey)
    const manifest = await signManifest(kp.privateKey, {
      members: ['Alice'], // a human roster IS present → the human gate stays on
      mode: 'invite',
      room: 'standup',
      exp: NOW + 3600,
      agentKeys: [{ key: agentJwk, label: 'Singer' }],
    ...
    const verify = await gateVerifierFor({ mode: 'invite', pubKey, manifest }, 'standup', () => NOW)
    expect((await verify(undefined, null)).ok).toBe(false) // tokenless human still denied
  })
