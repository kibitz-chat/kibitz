  ...
  return {
    claim(room) {
      ...
      const transport: AuthorityTransport = {
        ...
        // In-process: claim() is atomic (no broker/WS), so an authority is never re-claimed out from under itself —
        // onGone never fires here. Accept + ignore the handler to satisfy the interface.
        onGone: () => {},
