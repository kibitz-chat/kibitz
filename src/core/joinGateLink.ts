...
const GATE_KEYS = ['g', 'gn', 'gk', 'gm', 'ge', 'gc', 'gt', 'gh', 'ghk', 'ghn', 'gho', 'gl', 'gd', 'd', 'ag', 'n'] as const
...
export interface GateDescriptor {
  ...
  hostEmail?: string
  /** Open-room CLAIM list (M — docs/claimed-identity.md): the invited identities a joiner PICKS from to
   *  DECLARE who they are (UNVERIFIED). Emails + an optional literal "Guest". Applies to an OPEN room and
   *  is display-only — it never gates. The verified ✓ stays the only proof (a separate, optional sign-in). */
  declare?: string[]
}
...
export function encodeGateParams(d: GateDescriptor): URLSearchParams {
  ...
  if (d.declare?.length) p.set('gd', d.declare.join(','))
  return p
}
...
export function decodeGateParams(p: URLSearchParams): GateDescriptor {
  ...
  // The open-room CLAIM list applies to ANY mode incl. open, so read it BEFORE the open-mode early return.
  const gd = p.get('gd')
  if (gd) d.declare = gd.split(',').map((s) => s.trim()).filter(Boolean)
