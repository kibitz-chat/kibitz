...
describe('verifyManifest — an agent-only manifest (empty members, agentKeys) gates agents, humans open', () => {
  // The "start a room with an AI agent" shape: `mode:'invite'`, NO human members (humans stay open),
  // and an `agentKeys` allow-list. The authority must load it with NO mode pin — agent admission is
  // orthogonal to OIDC/Google (the bug was the widget reading agentKeys only in the google branch).
  // The agent then proves its key with a cert-bound assertion (agentKey.ts).
  const agentKey = { kty: 'EC', crv: 'P-256', x: 'fake-x', y: 'fake-y' } as unknown as JsonWebKey

  it('verifies WITHOUT a mode pin and yields agentKeys as the allow-list', async () => {
    const kp = await generateInviteKeypair()
    const m = manifest({ members: [], agentKeys: [{ key: agentKey, label: 'notes-bot' }] })
    const token = await signManifest(kp.privateKey, m)
    const r = await verifyManifest(token, kp.publicKey, { room: 'standup', now: NOW })
    expect(r.ok).toBe(true)
    expect(r.ok && r.manifest.agentKeys).toHaveLength(1)
    expect(r.ok && r.manifest.agentKeys?.[0].label).toBe('notes-bot')
  })

  it('agentKeys alone satisfy "has an allow-list" — empty members + no mh is NOT rejected', async () => {
    const kp = await generateInviteKeypair()
    const token = await signManifest(kp.privateKey, manifest({ members: [], mh: [], agentKeys: [{ key: agentKey }] }))
    const r = await verifyManifest(token, kp.publicKey, { room: 'standup', now: NOW })
    expect(r.ok).toBe(true)
  })
})
