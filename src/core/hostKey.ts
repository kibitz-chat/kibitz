...
export interface HostCommandPayload {
  ...
  iat: number
  /** Unique command id (random). The verifier LRU-dedups it to block a captured command being REPLAYED on the
   *  same connection within the freshness window. Optional: an old client omits it (no replay guard, as before);
   *  a new client always sends it, so new commands ARE replay-guarded. */
  jti?: string
}
...
export function signHostCommand(
  ...
  const payload: HostCommandPayload = {
    ...
    iat: args.now,
    jti: Array.from(crypto.getRandomValues(new Uint8Array(12)), (b) => b.toString(16).padStart(2, '0')).join(''),
  }
...
export async function verifyHostCommand(
  token: string,
  args: HostVerifyArgs,
): Promise<{ ok: true; op: HostOp; target?: string; jti?: string } | { ok: false; reason: string }> {
