...
// and let tests deliver inbound messages + roster changes. `withSchemas`/`withWidgets` opt the
// controller into the (optional) discovery / widget surfaces so we can test both shapes.
function fakeController(roster: AgentParticipant[] = [], opts: { withSchemas?: boolean; withWidgets?: boolean } = {}) {
  ...
  const widgets: { kind: string; data: unknown; id: string }[] = []
  const tapCbs = new Set<(m: AgentWidgetTap) => void>()
  let widSeq = 0
  ...
  const controller: AgentController = {
    ...
    ...(opts.withWidgets
      ? {
          sendWidget: (kind: string, data: unknown, id?: string) => {
            const wid = id || `w${++widSeq}`
            widgets.push({ kind, data, id: wid })
            return wid
          },
          onWidgetEvent: (cb: (m: AgentWidgetTap) => void) => (tapCbs.add(cb), () => tapCbs.delete(cb)),
        }
      : {}),
  }
  return {
    ...
    deliverTap: (m: AgentWidgetTap) => tapCbs.forEach((cb) => cb(m)),
...
describe('createAgent — bounded interactive widgets (map)', () => {
  it('showMap posts a kbz.map widget and returns the instance id', () => {
    const f = fakeController([], { withWidgets: true })
    const a = createAgent(f.controller)
    const id = a.showMap({ center: { lat: 32, lng: 34 }, zoom: 12 })
    expect(id).toBe('w1')
    expect(f.widgets).toEqual([{ kind: 'kbz.map', data: { center: { lat: 32, lng: 34 }, zoom: 12 }, id: 'w1' }])
  })

  it('showWidget honours a caller-supplied instance id and onWidgetTap receives interactions', () => {
    const f = fakeController([], { withWidgets: true })
    const a = createAgent(f.controller)
    expect(a.showWidget('kbz.map', { center: { lat: 0, lng: 0 }, zoom: 3 }, 'fixed')).toBe('fixed')
    const taps: AgentWidgetTap[] = []
    a.onWidgetTap((m) => taps.push(m))
    f.deliverTap({ from: 'p2', id: 'fixed', e: { t: 'pin', pin: { id: 'x', lat: 1, lng: 2 } } })
    expect(taps).toEqual([{ from: 'p2', id: 'fixed', e: { t: 'pin', pin: { id: 'x', lat: 1, lng: 2 } } }])
  })

  it('degrades to a no-op id / no-op subscribe when the controller predates widgets', () => {
    const f = fakeController() // no withWidgets
    const a = createAgent(f.controller)
    expect(a.showMap({ center: { lat: 0, lng: 0 }, zoom: 1 })).toBe('') // no id available, but no throw
    const unsub = a.onWidgetTap(() => {
      throw new Error('should never fire')
    })
    expect(() => unsub()).not.toThrow()
  })

  it('refuses to post when read-only', () => {
    const f = fakeController([], { withWidgets: true })
    const a = createAgent(f.controller, { readOnly: true })
    expect(() => a.showMap({ center: { lat: 0, lng: 0 }, zoom: 1 })).toThrow(/read-only/)
