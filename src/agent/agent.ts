...
import { MAP_WIDGET_KIND } from '../widget/mapWidget'
...
/** An interaction with a bounded widget the agent posted (e.g. a viewer dropping a pin on its map),
 *  attributed by the roster. `e` is the renderer-defined event shape. */
export interface AgentWidgetTap {
  from: string
  id: string
  e: unknown
}
...
export interface AgentController {
  ...
  // Bounded interactive widgets (optional — older controllers omit them). Lets an agent post a
  // pressable widget (a map) and react to the interactions peers make on it.
  sendWidget?(kind: string, data: unknown, id?: string): string
  removeWidget?(id: string): void
  onWidgetEvent?(cb: (m: AgentWidgetTap) => void): () => void
}
...
export interface AgentSession {
  ...
  /** Post a BOUNDED interactive widget into the room (a first-party renderer draws it from this validated
   *  payload — the agent never ships code). Returns the instance id. No-op id when the controller predates
   *  widgets. Disabled when readOnly. */
  showWidget(kind: string, data: unknown, id?: string): string
  /** Convenience over showWidget: post a MAP. `data` is `{ center:{lat,lng}, zoom, markers?, title? }`. */
  showMap(data: unknown, id?: string): string
  /** Retract a widget this agent posted (e.g. an image that failed to render in the call) — peers drop it. */
  removeWidget(id: string): void
  /** Subscribe to interactions on widgets this room shows (e.g. a viewer dropped a pin) so the agent can
   *  react. Returns an unsubscribe fn (no-op if the controller predates widgets). */
  onWidgetTap(cb: (m: AgentWidgetTap) => void): () => void
...
export function createAgent(controller: AgentController, opts: AgentOptions = {}): AgentSession {
  ...
  return {
    ...
    showWidget: (kind, data, id) => {
      if (readOnly) throw new Error('Kibitz agent is read-only — actions are disabled')
      return controller.sendWidget?.(kind, data, id) ?? (id || '')
    },
    showMap: (data, id) => {
      if (readOnly) throw new Error('Kibitz agent is read-only — actions are disabled')
      return controller.sendWidget?.(MAP_WIDGET_KIND, data, id) ?? (id || '')
    },
    removeWidget: (id) => {
      if (readOnly) return
      controller.removeWidget?.(id)
    },
    onWidgetTap: (cb) => controller.onWidgetEvent?.(cb) ?? (() => {}),
    canAct: !readOnly,
