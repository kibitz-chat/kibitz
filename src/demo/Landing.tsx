    ...
    <main className="landing">
      <section className="hero">
        ...
        <div className="start-row">
          ...
          {brand.secondaryCta && (
            <a className="start start-secondary" href={brand.secondaryCta.href}>
              {brand.secondaryCta.label}
            </a>
          )}
        </div>
        <p className="hint">free · no signup · works on phones</p>
        {installed && onOpen ? (
          ...
          <OpenRoom onOpen={onOpen} extra={<Settings />} />
        ) : (
 ...
 */
function Launcher({ onStart, onOpen }: { onStart: (desc?: string) => void; onOpen?: (input: string) => boolean }) {
  return (
    <main className="launcher">
      <section className="hero">
        ...
        <p className="hint">free · joins muted, camera off</p>
        {onOpen ? (
          <>
            <div className="launcher-or" aria-hidden="true">
              or join one you were sent
            </div>
            <OpenRoom onOpen={onOpen} expanded extra={<Settings />} />
          </>
        ) : (
          <div className="entry-row">
            <Settings />
...
function OpenRoom({
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!onOpen(val)) setErr('invalid') // invalid → stay and hint (a valid one navigates away)
  }
  ...
  if (!open) {
    return (
      <div className="entry-row">
        <button className="open-toggle" type="button" onClick={() => setOpen(true)}>
    ...
    return (
      <div className="open-form">
        <Suspense fallback={<p className="hint">Loading scanner…</p>}>
          <QrScanner onScan={onOpen} onClose={() => setScan(false)} />
        </Suspense>
      </div>
    )
  }
    ...
    <form className="open-form" onSubmit={submit}>
        ...
        <div className="entry-row">
          {canPaste && (
            <button className="open-paste" type="button" onClick={pasteJoin}>
              {armed ? '📋 Tap again to paste' : '📋 Paste link & join'}
            </button>
          )}
          {extra}
        </div>
      ...
      <div className="open-row">
        <input
          ...
          spellCheck={false}
          inputMode="url"
          // eslint-disable-next-line jsx-a11y/no-autofocus -- the user just chose to open this field
          autoFocus
        />
        <button className="open-go" type="submit">
          Join
        </button>
      </div>
      {err === 'invalid' ? (
        ...
        <p className="hint">Paste the invite link someone sent you — it opens right here in the app.</p>
      )}
