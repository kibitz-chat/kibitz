...
import { stripControlSecrets, stripControlSecretsFromUrl, summonKeyForRoom } from '../core/memKey'
import { recordRecentRoom } from '../core/recentRooms'
...
import { safeReturnUrl } from '../widget/returnUrl'
import { brand } from '../brand'
import { isSingleInstanceDormant } from '../react/useSingleInstance'
...
// Offline/LAN rooms aren't a reserved route — they're a normal room id in the hash with the hub's blob in the
// ?galaxy= query (an "offline-room link" the creator shares). The room branch keys on urlGalaxyBlob() to know a
// room is offline; a bare room hash with no blob stays online as before.
...
export function App() {
  ...
  // The open-room DECLARE/claim list (M) is the one gate field that must flow after an IN-APP create (a hash
  // change, no reload) — so re-read it from the CURRENT hash each render, like the display params above, and
  // merge it into the descriptor the room mounts with. (The crypto gate fields still come from a fresh load.)
  const liveDeclare = ((): string[] | undefined => {
    const gd = gateParamsFrom(location.hash, location.search).get('gd')
    return gd ? gd.split(',').map((s) => s.trim()).filter(Boolean) : undefined
  })()
  const gateLive = liveDeclare?.length ? { ...gate, declare: liveDeclare } : gate
  ...
  // This URL is an OFFLINE-room link: the hub's blob rides in ?galaxy= and the room id in the hash. Keyed on the
  // URL blob (not the localStorage cache) so a blob cached from an earlier offline call can't hijack an online room.
  // Offline rooms are CREATED from the room-setup page's Advanced "Use this Wi-Fi's relay" toggle (see CreatePage).
  const offlineRoom = !!urlGalaxyBlob()
  ...
  // An OFFLINE room: the hub's blob is in ?galaxy= (the shared link carries it) and the room id is the hash.
  // Connect straight to the hub from the blob — no discovery, the link already names it — and join that room.
  // Open (skips the online gates below): the unguessable room id IS the capability, and there's no signaling
  // server here to enforce a gate against anyway.
  if (roomName && offlineRoom) return <RoomPage key={roomName} roomName={roomName} roomDesc={roomDesc} gate={gateLive} offline />
  ...
  if (roomName) return <RoomPage key={roomName} roomName={roomName} roomDesc={roomDesc} notice={roomNotice} agentCall={agentCall} gate={gateLive} />
  if (creating)
    return (
      <CreatePage
  ...
  return (
    <Landing
      // The offline/host shell (the native APK — always a LAN hub via ?galaxy=) shows the LAUNCHER, not the
      // marketing page: it's a using-the-app context, so it should open the SAME home as the installed PWA,
      // not an acquisition landing. (A browser galaxy/LAN user gets the launcher too — also a using context.)
      // Key on the URL's ?galaxy= (urlGalaxyBlob), NOT hasGalaxy(): the latter reads the localStorage cache, so a
      // blob left over from an earlier offline/LAN call would hijack EVERY normal online visit into the launcher
      // (matches the room branch above, which already keys on urlGalaxyBlob — see galaxyHub.ts:46-48).
      standalone={standalone || !!urlGalaxyBlob()}
      installed={installed}
      ...
      onOpen={(input) => {
        // A brand SHORT link (e.g. kibitz.chat/r/<code>) hides the room behind a server redirect, so the
        // parser below can't read a room out of it. When this brand runs that service (brand.shortlinkApi),
        // just NAVIGATE to the short link and let the browser follow the redirect to the real room — so a
        // scanned/pasted short link works, not only a tapped one. Accept /r/<code> on OUR origin OR the
        // shortlink service's origin: the code is minted where the /r resolver lives (e.g. the apex
        // kibitz.chat), which differs from the app's origin (e.g. www.kibitz.chat) after the Cloudflare
        // split — WITHOUT this, a scanned QR fails the same-origin check and "can't be read." Both are trusted
        // (we run the shortener), so it's no open-redirect.
        if (brand.shortlinkApi) {
          try {
            ...
            if ((u.origin === location.origin || u.origin === shortOrigin) && /^\/r\/[A-Za-z0-9]{4,16}\/?$/.test(u.pathname)) {
              location.assign(u.href)
              return true
            }
          } catch {
            /* not a URL — fall through to the normal room parse */
          }
        }
...
function RoomPage({
  ...
  // after joining. Only once IN the call does the surface go black. An OFFLINE/LAN room shows the SAME
  // light paper pre-join as any other room (it does NOT join straight away — key on `joined`, not `offline`,
  // or the offline pre-join renders dark and looks nothing like the usual one). Paint <html> too: a
  // drag-resize repaints the bare root before the content reflows.
  useEffect(() => {
    // Auto-join rooms (the "Tap to join & talk" gate) go straight into the call as one DARK hop — there is no light
    // lobby to show. Keep the document dark for them even PRE-JOIN: otherwise iOS Safari bleeds the light paper `html`
    // background through at the screen edges behind the gate's dark tile (the hairline white side-lines).
    const autoJoin = splitRoomHash(location.hash).params.get('autojoin') === '1'
    const dark = joined || autoJoin
  ...
  // Agent-path short links are cached (long → short) so repeated copies don't mint a fresh code each time.
  const shortCache = useRef<{ long: string; short: string } | null>(null)
  const buildInvite = async (): Promise<string> => {
    const key = getLicenseKey()
    const grant = key ? (await requestRoomGrant(roomName, key))?.grant : null
    let link: string
    if (!offline && (gate.mode === 'open' || brand.shareGatedViaJHop)) {
      let l = `${location.origin}/j/${encodeURIComponent(roomName)}`
      // Carry params through the /j hop on the QUERY (the fragment can't survive it) — both our /j hops
      // forward the query, and the app reads gate params from the query when the fragment has none
      // (gateParamsFrom). For a GATED room (shareGatedViaJHop) seed from the current link's params — the
      // committed gate (g/gk/gm) + summon key (sk) — so the shared link still admits the agent. The
      // notice/type aren't secret; the gate/sk tradeoff is the opt-in's whole point (see brand.ts).
      const cp = new URLSearchParams(splitRoomHash(location.hash).params)
      // …but after JOINING via a /j link, those gate params (g/gk/gm), the summon key (sk) and the agent id
      // live on the QUERY (the /j hop forwards them there; the fragment then holds only the room). Seeding
      // from the fragment alone DROPS them — so the copied/re-shared link can't admit the recipient to a
      // gated room and they land in a half-state (split roster inconsistency). Carry the query
      // params over too. Skip 'k' (a possibly-stale TURN grant) — a fresh one is minted above.
      for (const [qk, qv] of new URLSearchParams(location.search)) if (qk !== 'k' && !cp.has(qk)) cp.set(qk, qv)
      ...
      // A rebrand can send the ORGANIZER back to one place on leave (their own link's `back=`) but everyone they
      // INVITE somewhere else — witz's gift flow returns guests to the keepsake page (open+share, no dashboard),
      // passed as `gback=`. Promote it to the shared invite's `back=` (and drop the organizer's own back + gback).
      const gback = cp.get('gback')
      if (gback) {
        cp.set('back', gback)
        cp.delete('gback')
      }
      // Strip control secrets (sk/mk/st) from the SHARED invite — only the summoner's OWN link keeps them, so an
      // invitee can join but not summon or read memory (docs/encrypted-memory.md — the summoner model).
      const q = stripControlSecrets(cp).toString()
      if (q) l += `?${q}`
      link = grant ? linkWithGrant(l, grant) : l
    } else {
      // Same here: never share sk/mk — strip them from the full current URL (query + room fragment).
      let href = grant ? linkWithGrant(location.href, grant) : location.href
      // OFFLINE host in the native APK runs under the bundled virtual origin (appassets.androidplatform.net),
      // which a guest's browser can't open. Swap it for the REAL site origin the shell stashed in localStorage,
      // so the shared ?galaxy= link loads kibitz.chat (from the guest's cache) and reaches the same hub.
      const BUNDLED = 'https://appassets.androidplatform.net'
      if (href.startsWith(BUNDLED)) {
        let site: string | null = null
        try {
          site = localStorage.getItem('kbz.siteOrigin')
        } catch {
          /* no storage — leave the link as-is */
        }
        if (site) href = site.replace(/\/+$/, '') + href.slice(BUNDLED.length)
      }
      link = stripControlSecretsFromUrl(href)
    }
    // AGENT path on a brand with a shortener (kibitz): the invite IS the SHORT link everywhere — every copy
    // button (in-call, the QR panel, the pre-join) and the QR resolve through buildInvite. The regular path
    // keeps the full link. Cache by the long link so repeated copies reuse one code instead of minting more.
    if (agentCall && brand.shortlinkApi) {
      if (shortCache.current?.long === link) return shortCache.current.short
      try {
        const short = await makeShortLink(link)
        if (short && short !== link) {
          shortCache.current = { long: link, short }
          return short
        }
      } catch {
        /* shortener unreachable → fall back to the full link */
      }
    }
    return link
  }

  // Optional brand short-link: POST the full invite URL to the brand's OWN shortener (brand.shortlinkApi —
  // e.g. kibitz.chat/shorten), get back a short URL that redirects to it. Only wired into the widget when the
  // brand sets it, so the default product (kibitz) shows no button and makes no such request.
  const makeShortLink = async (url: string): Promise<string> => {
    if (!brand.shortlinkApi) return url
    const res = await fetch(brand.shortlinkApi, {
      method: 'POST',
  ...
  // The way OUT of a full-window room (no browser back button in an installed PWA / the native APK). Clear ONLY
  // the room (the hash) — App then re-renders the page BEFORE the room. Crucially KEEP the ?galaxy= query: a
  // native-hub APK (or any offline launcher) must return to its LAUNCHER, not drop its own hub and fall to the
  // marketing home (the earlier `origin + pathname` did exactly that — the reported bug). Online (no galaxy) this
  // still lands on the Landing. Used by the widget's exit (onExit) AND hang-up (the in-call → left transition
  // below), so leaving always returns you to the page before — never the pre-join lobby.
  const goHome = () => {
    location.hash = ''
  }
  ...
  useEffect(() => {
    // The summoner key: the URL's `sk` if present (and persist it for this room), else the per-room stash from a
    // prior visit — so leave→re-enter keeps the summon button even after goHome wipes the fragment. See memKey.
    const summonKey = summonKeyForRoom(roomName, location.hash, location.search)
    // Remember this room for the start page's "Recent rooms" list (re-enter after leaving). Local-only; the
    // display name is the room's description (the `d` fragment param) if it has one, else the bare code.
    try {
      recordRecentRoom({ code: roomName, name: splitRoomHash(location.hash).params.get('d') || roomName, hash: location.hash, at: Date.now() })
    } catch {
      /* storage blocked — non-fatal */
    }
    // A name entered on an UPSTREAM step (e.g. witz's pre-call page) and handed over via `nm` → pre-fills the
    // pre-join name field so the caller doesn't type it twice. Bounded; matches the field's own maxLength.
    const callerName = (splitRoomHash(location.hash).params.get('nm') || '').slice(0, 14)
    // `autojoin=1` (e.g. from witz's pre-call step) skips the pre-join lobby: the name + consent were taken
    // upstream, so land straight in the call (muted; mic opens on the first in-call unmute tap).
    const autoJoin = splitRoomHash(location.hash).params.get('autojoin') === '1'
    const w = mount({
      room: roomName,
      startOpen: true,
      ...
      ...(brand.menuOrigin ? { menuOrigin: brand.menuOrigin } : {}), // white-label: allow an in-call host menu on this origin (rating)
      ...(brand.summonPath ? { summonPath: brand.summonPath } : {}), // white-label: in-call "Summon agent" button → <path>?room=
      ...(brand.summonApi ? { summonApi: brand.summonApi } : {}), // white-label: one-tap summon endpoint (POST {summonKey})
      ...(summonKey ? { summonKey } : {}), // summon key — URL control link first (persisted per-room), else the per-room stash so leave→re-enter keeps it. Only the summoner has it; stripped invites don't.
      ...
      ...(gate.mode !== 'open' || gate.hostPubKey || gate.hostName || gate.hostEmail || gate.declare?.length ? { joinGate: gate } : {}), // also pass it for an OPEN room with a host (key/name/OIDC tier) or a declare/claim list (M)
      ...(selfCred ? { joinCredential: selfCred } : {}),
      offline: !!offline, // the per-room CHOICE (online vs LAN) — the Widget must honor THIS, not just "is a relay present"
      ...(!offline && getRelayOnly() ? { relayOnly: true } : {}),
      ...(brand.singleInstance ? { singleInstance: true } : {}), // white-label: one live call per browser (older tabs go dormant)
    })
    ...
    // When the room link carries a brand `back=` return URL (e.g. the witz gift dashboard), the WIDGET owns where
    // "leave" goes — it paints a splash and navigates straight there itself (see leaveCall/returnHome). If we ALSO
    // goHome() here we'd wipe the hash first, flashing the Landing behind that redirect AND unmounting the widget
    // out from under its own splash (the "homepage blinks on hang-up" bug). So skip goHome when a return URL is set.
    const hasReturnUrl = !!safeReturnUrl(
      splitRoomHash(location.hash).params.get('back') ?? new URLSearchParams(location.search).get('back'),
      brand.returnHosts,
    )
    let wasInCall = false
    const off = w.on('state', (s: { inCall: boolean }) => {
      setJoined(s.inCall)
      // Don't navigate home when the leave was INVOLUNTARY — a single-instance takeover in another tab
      // drops us from the call, but we keep the room so the "Use this tab" overlay can resume it.
      // And don't when a return URL is set — the widget carries the leave to the dashboard (no Landing flash).
      if (wasInCall && !s.inCall && !isSingleInstanceDormant() && !hasReturnUrl) goHome()
      wasInCall = s.inCall
    })
    return () => {
      off()
