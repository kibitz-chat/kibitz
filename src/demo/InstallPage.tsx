...
import { InstallSteps, type InstallKind } from './InstallSteps'
...
export function InstallPage({ onBack, onStart }: { onBack: () => void; onStart: () => void }) {
  ...
  // Where the VISUAL steps (glyph chips + a browser-bar mockup) apply: the mobile flows people get stuck on.
  // iOS Safari = Share at the bottom; iOS Chrome = Share top-right; Android = the ⋮ menu. Edge/Firefox-iOS,
  // desktop and Mac keep the text steps (their flows differ / are already clear).
  const ua = typeof navigator !== 'undefined' ? navigator.userAgent : ''
  const visualKind: InstallKind | null =
    plat === 'android' ? 'android' : plat === 'ios' ? (/CriOS/i.test(ua) ? 'ios-chrome' : /EdgiOS|FxiOS/i.test(ua) ? null : 'ios-safari') : null
  ...
  return (
    <main className="installpage">
      ...
      {/* Brand-gated: a native LAN-hub companion app (e.g. kibitz '/host'). Only shows where the brand sets it
          (the default product has no such app, so kibitz.chat never shows a broken link) AND only on Android —
          the APK installs nowhere else, so offering it on iOS / desktop / Mac is a dead end. Guests need nothing. */}
      {brand.offlineAppPath && plat === 'android' && (
        <section className="install-offline">
          <h2>Call with no internet</h2>
          <p>
            On one Android phone, get <strong>{brand.name}+</strong> — it adds <strong>offline</strong>: that
            phone becomes a local hub so everyone on the same Wi-Fi can call with <strong>no internet</strong>.
            Guests just open this site; only the host needs {brand.name}+.
          </p>
          <a className="start" href={brand.offlineAppPath}>
            📡 Get {brand.name}+ (Android)
          </a>
        </section>
      )}
