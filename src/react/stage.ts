...
interface Presentable {
  ...
  /** The presenter's screen-share stream (the dedicated share lane) — the stage renders THIS, not the camera. */
...
/**
 * The participant to stage: the newest presenter (highest `presentAt`) that actually has a SHARE stream to
 * show — or null when nobody is presenting (or the presenter's share hasn't arrived yet, nothing to render).
 */
export function pickPresenter<T extends Presentable>(participants: readonly T[]): T | null {
  let best: T | null = null
  for (const p of participants) {
    if (!isPresenting(p) || !p.shareStream) continue
