 ...
 * Returns `supported` (gate the button), `active`, and `toggle`.
 */
export function useVideoPip(getFocus: () => PipFocus | null): {
  ...
  const draw = useCallback(() => {
    ...
    if (f && f.hasVideo && f.stream) {
      ...
      if (rig.src.readyState >= 2 && rig.src.videoWidth) coverDraw(rig.ctx, rig.src)
  ...
  // Tear everything down on unmount (leaving the call): exit PiP, stop the canvas capture, remove
  // the off-screen elements.
  useEffect(
    () => () => {
      exit()
      const rig = rigRef.current
