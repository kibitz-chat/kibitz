...
export function QrScanner({ onScan, onClose }: { onScan: (text: string) => boolean; onClose: () => void }) {
  ...
  const [notRoom, setNotRoom] = useState(false) // scanned a QR that wasn't a Kibitz room
  ...
  const msg: Record<ScanStatus, string> = {
    ...
    scanning: notRoom ? 'That QR isn’t a Kibitz room — point at the room’s code.' : 'Point at a Kibitz room QR code.',
