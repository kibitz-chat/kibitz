...
const BRAND_ACCENT = process.env.VITE_BRAND_ACCENT || ''
...
const brandHtml = {
  ...
  transformIndexHtml(html: string) {
    ...
    if (IS_REBRAND) {
      html = html
        // Bake the brand accent + rebrand class onto <html> so the PRERENDERED first paint is already themed.
        // Otherwise the static landing flashes the default (Kibitz green) theme on a cold/hard reload until
        // main.tsx applies the accent at runtime — looked like "the wrong brand for half a second".
        .replace(/<html lang="en">/, `<html lang="en" class="brand-rebrand" style="--brand-accent: ${BRAND_ACCENT}">`)
        ...
        // Point the social-preview image at THIS brand's origin (each brand overlays its own icon-512.png).
        .replace(/(<meta property="og:image" content=")[^"]*(")/, `$1${BRAND_URL}/icon-512.png$2`)
        .replace(/(<meta name="twitter:image" content=")[^"]*(")/, `$1${BRAND_URL}/icon-512.png$2`)
        .replace(
...
export default defineConfig({
  ...
  plugins: [
    react(),
    brandHtml,
    // Emit /version.json = { version: BUILD_ID } so the running app can poll it (cache-disabled) and detect a
    // new deploy WITHOUT relying on the flaky iOS-Safari service-worker update lifecycle. `.json` is not in the
    // SW precache globs below, so it's always fetched from the network (never served stale from the cache).
    {
